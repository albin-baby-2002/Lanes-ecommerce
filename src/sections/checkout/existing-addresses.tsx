...
const ExistingAddresses = ({
  ...
  return (
    <AccordionItem value="item-1" className="border px-4">
      ...
      <AccordionContent className="flex max-h-[764px] w-full flex-wrap gap-3 overflow-y-auto border-t">
        {loading ? (
          ...
          >
            {Addressess?.map((address, idx) => {
              return (
                <div
                  key={idx}
                  className="flex w-full space-x-4 border-t-2 pt-4 first:border-t-0"
                >
                  <RadioGroupItem
                    className="mt-2"
